...
class Socket:
    ...
    async def send_data(self, **kwargs):
        try:
            ...
            await self.generel_loop.sock_sendall(where, meta_data + data)
    ...
    async def _recv_message(self, listened_socket: socket.socket, message_len: int):
        ...
        while len(message) < message_len:
            packet = await self.generel_loop.sock_recv(listened_socket, message_len - len(message))
            if packet is None:
                return None
    ...
    def start(self):
        ...
        self.generel_loop.run_until_complete(self.main())
